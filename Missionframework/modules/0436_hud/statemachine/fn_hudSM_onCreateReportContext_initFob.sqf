...
if (_debug) then {
    [format ["[fn_hudSM_onCreateReportContext_initFob] Entering: [isNull _player, isNull _context]: %1"
        , str [isNull _player, isNull _context]], "HUDSM", true] call KPLIB_fnc_common_log;
};
...
if (_debug) then {
    [format ["[fn_hudSM_onCreateReportContext_initFob] Fini: [_reportAllResources, count _fobs]: %1"
        , str [_reportAllResources, count _fobs]], "HUDSM", true] call KPLIB_fnc_common_log;
};
