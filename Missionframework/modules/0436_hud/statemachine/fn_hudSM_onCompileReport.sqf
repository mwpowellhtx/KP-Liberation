...
/*
    KPLIB_fnc_hudSM_onCompileReport

    File: fn_hudSM_onCompileReport.sqf
    Author: Michael W. Powell [22nd MEU SOC]
    Created: 2021-04-03 00:31:59
    Last Update: 2021-05-17 14:07:11
...
if (_debug) then {
    ["[fn_hudSM_onCompileReport] Entering...", "HUDSM", true] call KPLIB_fnc_common_log;
};
...
{
    if (_debug) then {
        [format ["[fn_hudSM_onCompileReport::forEach] Report callback: [owner _player, _forEachIndex]: %1"
            , str [owner _player, _forEachIndex]], "HUDSM", true] call KPLIB_fnc_common_log;
    };
...
if (_debug) then {
    [format ["[fn_hudSM_onCompileReport] Evaluating: [count _compiledReport]: %1"
...
if (_debug) then {
    [format ["[fn_hudSM_onCompileReport] Status: [_fobReportCount, _sectorReportCount]: %1"
        , str [_fobReportCount, _sectorReportCount]], "HUDSM", true] call KPLIB_fnc_common_log;
};
...
if (_debug) then {
    [format ["[fn_hudSM_onCompileReport] Fini: [_compiledStatus, count _standbyReport]: %1"
        , str [_compiledStatus, count _standbyReport]], "HUDSM", true] call KPLIB_fnc_common_log;
};
