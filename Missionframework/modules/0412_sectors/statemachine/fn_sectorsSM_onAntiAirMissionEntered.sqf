...
/*
    KPLIB_fnc_sectorsSM_onAntiAirMissionEntered

    File: fn_sectorsSM_onAntiAirMissionEntered.sqf
    ...
    Description:
        ANTI-AIR MISSION 'onStateEntered' event handler.
...
private _debug = MPARAMSM(_onAntiAirMissionEntered_debug);
...
if (_debug) then {
    [format ["[fn_sectorsSM_onAntiAirMissionEntered] Entering: [_markerName]: %1"
...
if (true) then {
    [_namespace, MSTATUS(_antiAirMission), { true; }, QMVAR(_status)] call KPLIB_fnc_namespace_setStatus;
...
if (_debug) then {
    [format ["[fn_sectorsSM_onAntiAirMissionEntered] Fini: [_markerName]: %1"
        , str [_markerName]], "SECTORSSM", true] call KPLIB_fnc_common_log;
};
