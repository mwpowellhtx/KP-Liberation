...
/*
    KPLIB_fnc_sectorsSM_onPatrolMissionEntered

    File: fn_sectorsSM_onPatrolMissionEntered.sqf
    ...
    Description:
        PATROL MISSION 'onStateEntered' event handler.
...
private _debug = MPARAMSM(_onPatrolMissionEntered_debug);
...
if (_debug) then {
    [format ["[fn_sectorsSM_onPatrolMissionEntered] Entering: [_markerName]: %1"
...
if (true) then {
    [_namespace, MSTATUS(_patrolMission), { true; }, QMVAR(_status)] call KPLIB_fnc_namespace_setStatus;
...
if (_debug) then {
    [format ["[fn_sectorsSM_onPatrolMissionEntered] Fini: [_markerName]: %1"
        , str [_markerName]], "SECTORSSM", true] call KPLIB_fnc_common_log;
};
