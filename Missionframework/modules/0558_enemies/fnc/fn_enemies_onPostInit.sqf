...
/*
    KPLIB_fnc_enemies_onPostInit

    File: fn_enemies_onPostInit.sqf
...
if (isServer) then {
    ["[fn_enemies_onPostInit] Initializing...", "POST] [ENEMY", true] call KPLIB_fnc_common_log;
};
...
if (isServer) then {
    ["[fn_enemies_onPostInit] Initialized", "POST] [ENEMY", true] call KPLIB_fnc_common_log;
};
