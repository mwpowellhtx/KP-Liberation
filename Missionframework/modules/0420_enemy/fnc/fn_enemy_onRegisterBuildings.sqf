...
/*
    KPLIB_fnc_enemy_onRegisterBuildings

    File: fn_enemy_onRegisterBuildings.sqf
...
if (_debug) then {
    [format ["[fn_enemy_onRegisterBuildings] Entering: [_markerName, markerText _markerName, count _buildings]: %1"
...
if (!([_namespace] call MFUNC(_allowBuildingDestruction))) exitWith {
    if (_debug) then {
        [format ["[fn_enemy_onRegisterBuildings] City or metropolis: [_markerName, markerText _markerName]: %1"
...
if (_debug) then {
    [format ["[fn_enemy_onRegisterBuildings] Fini: [_markerName, markerText _markerName, count _buildings]: %1"
        , str [_markerName, markerText _markerName, count _buildings]], "ENEMY", true] call KPLIB_fnc_common_log;
};
