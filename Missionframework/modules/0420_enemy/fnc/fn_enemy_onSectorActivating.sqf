...
if (_debug) then {
    [format ["[fn_enemy_onSectorActivating] Entering: [_markerName, markerText _markerName, count _buildings]: %1"
...
if (!([_namespace] call MFUNC(_allowBuildingDestruction))) exitWith {
    if (_debug) then {
        [format ["[fn_enemy_onSectorActivating] City or metropolis: [_markerName, markerText _markerName]: %1"
