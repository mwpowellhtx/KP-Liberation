...
if (_debug) then {
    [format ["[fn_enemy_onSectorCaptured] Entering: [_markerName, markerText _markerName]: %1"
...
[[_markerName] call MFUNC(_getSectorCaptureReward)] call {
    ...
    if (_debug) then {
        [format ["[fn_enemy_onSectorCaptured::call] Reward: [_markerName, markerText _markerName, _civRepReward, _sectorReward]: %1"
            , str [_markerName, markerText _markerName, _civRepReward, _sectorReward]], "ENEMY", true] call KPLIB_fnc_common_log;
    };
...
if (_debug) then {
    [format ["[fn_enemy_onSectorCaptured] Fini: [_markerName, markerText _markerName]: %1"
        , str [_markerName, markerText _markerName]], "ENEMY", true] call KPLIB_fnc_common_log;
};
