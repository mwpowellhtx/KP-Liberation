...
/*
    KPLIB_fnc_wounded_onSectorCaptured
...
if (_debug) then {
    [format ["[fn_wounded_onSectorCaptured] Entering: [_markerName, markerText _markerName, _aidReward, _aidCount, _civRepReward]: %1"
        , str [_markerName, markerText _markerName, _aidReward, _aidCount, _civRepReward]], "WOUNDED", true] call KPLIB_fnc_common_log;
};
...
if (!(_markerName in (KPLIB_sectors_city + KPLIB_sectors_metropolis))) exitWith {
    if (_debug) then {
        [format ["[fn_wounded_onSectorCaptured] No wounded to aid: [_markerName, markerText _markerName]: %1"
            , str [_markerName, markerText _markerName]], "WOUNDED", true] call KPLIB_fnc_common_log;
    };
    true;
};
...
if ((_aidReward == 0 && _civRepReward < 0) || (_aidReward > 0 && _netReward > 0)) exitWith {
    if (_debug) then {
        [format ["[fn_wounded_onSectorCaptured] Disqualified: [_markerName, markerText _markerName, _aidCount, _aidReward, _civRepReward, _netReward]: %1"
            , str [_markerName, markerText _markerName, _aidCount, _aidReward, _civRepReward, _netReward]], "WOUNDED", true] call KPLIB_fnc_common_log;
    };
    true;
};
...
if (_debug) then {
    [format ["[fn_wounded_onSectorCaptured] Fini: [_markerName, markerText _markerName, _aidCount]: %1"
        , str [_markerName, markerText _markerName, _aidCount]], "WOUNDED", true] call KPLIB_fnc_common_log;
};
