...
/*
    KPLIB_fnc_garrison_onAllocateOpforLightVehicles

    File: fn_garrison_onAllocateOpforLightVehicles.sqf
    Author: Michael W. Powell [22nd MEU SOC]
    Created: 2021-04-29 11:33:46
    Last Update: 2021-06-24 12:47:05
    License: GNU General Public License v3.0 - https://www.gnu.org/licenses/gpl-3.0.html
    Public: Yes

    Description:
        Calculates the LIGHT VEHICLES ALLOCATION during the SECTOR ACTIVATION GARRISON
        phase.
...
private _debug = MPARAM(_onAllocateOpforLightVehicles_debug)
    || (_sector getVariable [QMVAR(_onAllocateOpforLightVehicles_debug), false]);
...
if (_debug) then {
    [format ["[fn_garrison_onAllocateOpforLightVehicles] Entering: [_markerName, markerText _markerName, _sectorType]: %1"
...
if (_debug) then {
    [format ["[fn_garrison_onAllocateOpforLightVehicles] Components: [_dieSides, _dieTimes, _dieOffsets, _adjustedOpforStrengthRatio]: %1"
...
if (_debug) then {
    [format ["[fn_garrison_onAllocateOpforLightVehicles] Allocation: [_allocationCount, _dice]: %1"
...
_regimentMap set [QMVAR(_lightVehicleAllocation), _allocation];

if (_debug) then {
    [format ["[fn_garrison_onAllocateOpforLightVehicles] Fini: [count _allocation]: %1"
