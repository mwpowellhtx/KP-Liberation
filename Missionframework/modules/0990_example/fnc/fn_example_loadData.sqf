...
if (KPLIB_param_debug) then {
    ["Example module loading...", "SAVE"] call KPLIB_fnc_common_log;
};
...
if (_moduleData isEqualTo []) then {
    if (KPLIB_param_debug) then {
        ["Example module data empty, creating new data...", "SAVE"] call KPLIB_fnc_common_log;
    };
    // Nothing to do here
} else {
    // Otherwise start applying the saved data
    if (KPLIB_param_debug) then {
        ["Example module data found, applying data...", "SAVE"] call KPLIB_fnc_common_log;
    };
};
